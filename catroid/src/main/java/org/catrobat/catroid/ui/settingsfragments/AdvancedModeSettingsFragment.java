...
public class AdvancedModeSettingsFragment extends PreferenceFragment {
	...
	public void onStop() {
		super.onStop();
		changeLanguage(SettingsFragment.isCatblocksAdvancedModeLanguageEnabled(getContext()));
	}
	...
	private void changeLanguage(@NonNull Boolean enabled) {
		...
		if (enabled) {
			...
			Toast.makeText(getContext(), "Language checked", Toast.LENGTH_SHORT).show();
		} else {
			if (!currentLanguage.equals(englishTag) || savedLanguage.equals(englishTag) || savedLanguage.isEmpty() || currentLanguage.equals(savedLanguage)) {
				return;
			}
			...
			getActivity().finishAffinity();
			new Thread(() -> inject(ProjectsCategoriesSync.class).getValue().sync(true));
			Toast.makeText(getContext(), "Language unchecked", Toast.LENGTH_SHORT).show();
		}
	}
}
